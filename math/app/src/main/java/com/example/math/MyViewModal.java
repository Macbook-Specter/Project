...
public class MyViewModal extends AndroidViewModel {
    ...
    public  MyViewModal(@NonNull Application application, SavedStateHandle handle) {
        super(application);
    ...
    public MutableLiveData<Integer>getRightNumber(){ return  handle.getLiveData(KEY_RIGHT_NUMBER); }
    ...
    public MutableLiveData<Integer>getCurrentScore (){ return  handle.getLiveData(KEY_CURRENT_SCORE); }
       MutableLiveData <Integer>getAnswer (){ return  handle.getLiveData(KEY_ANSWER); }
    void generator(){
        int LEVEL=20;
    ...
    void save(){
        ...
        editor.putInt(KEY_CURRENT_SCORE,getCurrentScore().getValue());
        editor.apply();
    }
